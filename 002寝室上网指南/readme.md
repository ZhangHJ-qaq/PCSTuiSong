...
以下是2021年御三家的宽带信息，在学校迎新服务网站上可以找到。

[电信](http://freshman.fudan.edu.cn/cms/view/wz.action?id=8aee956c7b9be9d7017ba017d0be3bc1) [移动](http://freshman.fudan.edu.cn/cms/view/wz.action?id=8aee956c7b9be9d7017ba0351bff4589) [联通](http://freshman.fudan.edu.cn/cms/view/wz.action?id=8aee956c7b9be9d7017ba02ff64f43c6) [学校自营宿舍网络](http://freshman.fudan.edu.cn/cms/view/wz.action?id=8aee956c7b9be9d7017ba040136248b1)

## 购买宽带后

如果同学购买的宽带支持**ifudan.stu**(应该是学校自营宿舍网络和移动)，可以直接用手机和电脑连接，在浏览器中输入对应的账号密码验证后即可上网。如果能找到ifudan.stu，在寝室里购买和安装路由器就**不是必须的**，你可以直接**关闭本文**。当然，你也可以选择安装一个，这样全寝室只需要购买一份宽带就可以了。安装后验证一次，只要路由器不重启就无需再次进行验证。
...
首先你得有**至少一条网线**，有可能购买路由器的时候会随机附赠。如果没有附赠，可以去网上或者数码城买一条，一般几元到十几元就可以买到了。网线两端的接口如下所示。如果你在安装路由器后，还想有线上网的（而不是使用Wi-Fi），那么你可能需要更多的网线。![network_cable](./pictures/network_cable.jpg)

### 连接路由器

这部分教程主要介绍如何连线。一般适用于大多数路由器型号。

以下是一台路由器背面的示意图。首先插上路由器的电源。将网线的一端插入书桌下面的网口，另一端插入路由器背面的WAN口。如下图所示 ：
...
当然，有一些路由器支持WAN/LAN的**盲插**，也就是不分WAN/LAN口，这样的话，直接插就完事咯！
...
按照提示配置【管理员密码】，当且仅当你拥有路由器的管理员密码，才可以访问路由器的管理界面对路由器进行管理，比如修改Wi-Fi的密码。如果管理员密码**忘了**，就只能将**路由器恢复出厂设置**来清除管理员密码了。设置完后点击下一步。
...
设置完后点击确定。路由器可能将会重启，稍等一会后你会发现你的Wi-Fi**已经不是开放**的了。你需要用你**刚才设置好的密码**连接你的Wi-Fi，如果连接后手机/电脑能够成功打开~~联网检测工具~~**百度**，说明设置成功。配置完之后第一次打开网页有可能会弹出一次认证页面，一般只要认证一次，之后只要不重启路由器
